...
# Get the original play_type column before get_dummies (reload it)
df_original = pd.read_csv("KC.csv")
df_original['injury'] = df_original['desc'].str.contains('injured|injury', case=False, na=False).astype(int)
df_original = df_original.dropna(subset=["play_type"])

# Calculate injury rate per play type
play_type_rates = df_original.groupby('play_type')['injury'].mean()

# Focus on run and pass
baseline_rate = df_original['injury'].mean()  # Overall injury rate as baseline

target_play_types = ['run', 'pass']
multipliers = {}

for play_type in target_play_types:
    if play_type in play_type_rates.index:
        raw_multiplier = play_type_rates[play_type] / baseline_rate
        # Shrink to range [0, 2]
        multipliers[play_type] = round(min(max(raw_multiplier, 0), 2), 4)
    else:
        print(f"  Warning: '{play_type}' not found in data")

print(f"\nBaseline injury rate (all plays): {baseline_rate:.4%}")
print(f"\nPlay Type Multipliers (0–2 scale):")
for play_type, multiplier in multipliers.items():
    rate = play_type_rates.get(play_type, 0)
    print(f"  {play_type.upper():>6}: {multiplier:.4f}  (injury rate: {rate:.4%})")